...
<p align="center">
  <img src="https://img.shields.io/badge/Version-v1.10.0-1D9BF0?style=flat-square" alt="Version 1.10.0" />
...
- [六、 科学计算、动态图表与富文本增强排版](#六-科学计算动态图表与富文本增强排版)
  ...
  - [6.3 语法高亮代码块与一键无损复制](#63-语法高亮代码块与一键无损复制)
    - [【案例 6.3】查看语法胶囊、一键复制并接收绿色 `✓ 已复制` 动效反馈](#案例-63查看语法胶囊一键复制并接收绿色--已复制-动效反馈)
  - [6.4 GFM 任务清单与高对比度表格交互](#64-gfm-任务清单与高对比度表格交互)
    - [【案例 6.4】构建项目敏捷冲刺任务看板与性能指标对比表](#案例-64构建项目敏捷冲刺任务看板与性能指标对比表)
- [七、 知识网络双向链接、块级互联与全景拓扑图谱](#七-知识网络双向链接块级互联与全景拓扑图谱)
  - [7.1 双向链接 `[[WikiLink]]` 与已链引用](#71-双向链接-wikilink-与已链引用)
    - [【案例 7.1】利用反向链接面板自动聚合各章节上下文并精准溯源](#案例-71利用反向链接面板自动聚合各章节上下文并精准溯源)
  - [7.2 ⚓ 块级原子互联与内联嵌入 (`^block-id`, `[[#^block]]`, `![[#^block]]`)](#72--块级原子互联与内联嵌入-block-id-block-block)
    - [【案例 7.2】跨文档段落精准微米级跳转与卡片内嵌式复用](#案例-72跨文档段落精准微米级跳转与卡片内嵌式复用)
  - [7.3 未链接提及智能扫描与一键升级双链](#73-未链接提及智能扫描与一键升级双链)
    - [【案例 7.3】一键将散落文本转化为正式双链节点](#案例-73一键将散落文本转化为正式双链节点)
  - [7.4 2.2ms 黄金螺旋 Cytoscape 全景力导向图谱](#74-22ms-黄金螺旋-cytoscape-全景力导向图谱)
    - [【案例 7.4】探索知识网络聚类、过滤孤岛节点并双击穿梭跳转](#案例-74探索知识网络聚类过滤孤岛节点并双击穿梭跳转)
...
- [十、 媒体与架构图无损灯箱 (Media Lightbox)](#十-媒体与架构图无损灯箱-media-lightbox)
  ...
  - [10.2 Mermaid 架构图 3× Retina 超清 PNG 导出](#102-mermaid-架构图-3-retina-超清-png-导出)
    - [【案例 10.2】一键无损导出 300+ DPI 矢量级透明底图用于演讲幻灯片](#案例-102一键无损导出-300-dpi-矢量级透明底图用于演讲幻灯片)
...
4. **顶部辅助工具栏（Toolbar）**：当前文档标题、四态视图胶囊切换（`阅读 / 分屏 / 源码 / 脑图`）、保存/另存为、上一篇/下一篇、正文行号开关（`#`）、打字机模式开关（`Alt+T`）、阅读字号无级滑块（0.85× ~ 1.35×）。
...
#### 【案例 6.3】查看语法胶囊、一键复制并接收绿色 `✓ 已复制` 动效反馈
...
### 7.2 ⚓ 块级原子互联与内联嵌入 (`^block-id`, `[[#^block]]`, `![[#^block]]`)
...
### 7.3 未链接提及智能扫描与一键升级双链
系统后台自动扫描全库文本，发现文章内容中出现过当前文档标题、但尚未添加 `[[...]]` 的“潜在关联”。

#### 【案例 7.3】一键将散落文本转化为正式双链节点
- **实操过程**：
...
### 7.4 2.2ms 黄金螺旋 Cytoscape 全景力导向图谱
点击活动栏的 🕸️「知识网络全景图谱」按钮或按下 `Ctrl + G`，展开全屏拓扑图：

![知识网络全景图谱](manual-images/21-global-graph.png)

- **2.2ms 黄金螺旋算法**：孤岛节点疏密有致环绕发散，关联紧密笔记自动聚合成星系簇。
- **发光波纹聚焦**：点击顶部「聚焦当前」按钮，平滑运镜至当前阅读文档并激发出 1.5 秒柔和天蓝脉冲光晕。
- **10% ~ 500% 无级缩放**：工具栏支持滚轮缩放或手动键入精确缩放百分比。

#### 【案例 7.4】探索知识网络聚类、过滤孤岛节点并双击穿梭跳转
- **实操过程**：
...
#### 【案例 10.2】一键无损导出 300+ DPI 矢量级透明底图用于演讲幻灯片
- **实操过程**：
...
| **导图与视图** | `Ctrl + M` | **思维导图视图** | 一键在 Markdown 读写与交互式思维导图间切换 |
...
| | `Ctrl + G` | **知识网络全景图谱** | 全屏查看知识节点关联拓扑与漫游定位 |
